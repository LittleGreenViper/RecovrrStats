...
struct RCVST_Chart5View: RCVST_DataDisplay, RCVST_UsesData {
    ...
    var body: some View {
        GeometryReader { inGeometry in
            GroupBox(title) {
                VStack(spacing: 8) {
                    NewLoginChart(data: $data, dataWindow: $dataWindow, selectedValuesString: $selectedValuesString)
                        .frame(
...
struct NewLoginChart: View, RCVST_UsesData, RCVST_HapticHopper {
     ...
     */
    private func _isLineDragged(_ inRowData: RCVST_DataProvider.RowFirstTimeLoginsPlottableData) -> Bool {
    ...
    var body: some View {
        ...
        .chartXAxisLabel("SLUG-BAR-CHART-Y-AXIS-NEW-LOGINS-LABEL".localizedVariant, alignment: .top)
        .chartXAxis {
